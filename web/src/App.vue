...
import { webPath } from "@/lib/base-path";
...
const logout = async () => {
	try {
		const response = await fetch(webPath("/auth/logout"), { method: "POST", credentials: "same-origin" });
...
<template>
  ...
  <div v-else-if="signedOut" class="login">
    <div class="login-card">
      ...
      <div class="grid gap-3"><Button v-for="provider in session?.providers" :key="provider.id" as-child variant="outline" class="justify-between h-12"><a :href="webPath(`/auth/login/${provider.id}`)"><span class="__">Continue with <span class="__var">{{ provider.name }}</span></span><ArrowRight class="size-4"/></a></Button></div>
